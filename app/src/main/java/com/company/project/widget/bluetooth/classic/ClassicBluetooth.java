package com.company.project.widget.bluetooth.classic;
...
public class ClassicBluetooth implements IBluetooth {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if (!deviceExist(device)) {
                    mDevices.add(device);//把所以搜索到的蓝牙MAC放到集合中
                }
                ...
                if (mListener != null) {
                    ...
                    if (mDevices.isEmpty()) {
                        ...
                        mListener.onDevicesFound(mDevices);//搜索到的蓝牙展示
                    }
                }
            }
        }
    ...
    private class ConnectedThread extends Thread {
        ...
        List<String> readMessageList = new LinkedList<>();
        ...
        public void run() {
            ...
            while (mState == STATE_CONNECTED) {
                ...
                try {
                    ...
                    KLog.e("结束读取流…………………………………………………………");
                    final String readMessage = new String(buffer, 0, length).replace(" ","");
                    KLog.e("读取的字节"+readMessage + "长度"+length);
                    //region 代码添加修改，解决数据接收断断续续
                    readMessageList.add(readMessage.trim().replace(" ",""));
                    final String message = StringUtils.concat(readMessageList);
                    if (message.endsWith("||")){
//                    float interval = Math.abs(System.currentTimeMillis() - nowTime) / 1000f; // 两次定位的时间间隔，单位：秒
//                    KLog.e("时间" + interval);
//                    if (interval > 10){
