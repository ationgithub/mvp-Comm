package com.company.project.mvp.view.main.setting;
...
import com.company.project.mvp.presenter.setting.BluetoothPresenter;
...
public class BluetoothFragment extends BaseFragment<BluetoothContract.Presenter> implements BluetoothContract.View {
    ...
    public void onDataReceived(String str) {
        ...
        if (str.length() > 0) {
            //通过指定字符进行截取
            String R = str.substring(str.indexOf("R")+1,str.indexOf("m"));
            //通过字节长度区间进行截取
            String HD = str.substring(35,40);
            KLog.e("读取的高度"+R + "前后距离"+HD);
            listResponse.add(0, str+"  " +"读取的高度:"+R +"   "+ "前后距离:"+HD);
            mAdapter.notifyDataSetChanged();
        }
    }
}
